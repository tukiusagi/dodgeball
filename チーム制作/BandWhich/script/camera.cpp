...
void UpdateCamera(void)
{
	...
	for (int nCnt = 0; nCnt < MAX_CAMERA; nCnt++)
	{
		...
		// アナログスティック右の取得
		GetJoypadStickRight(nCnt, &nStickH, &nStickV);

		g_camera[nCnt].posRDest.x = -25;
		g_camera[nCnt].posRDest.y = 90;
		g_camera[nCnt].posRDest.z = 25;

		// 角度が一定値を超えた場合の数値制限
		if (g_camera[nCnt].rotDest.y > D3DX_PI)
		{
			g_camera[nCnt].rotDest.y -= D3DX_PI * 2;
		}
		...
		if (g_camera[nCnt].bWraparound == true)
		{
			if (g_camera[nCnt].rotDest.y - g_camera[nCnt].rot.y < 0.01 && g_camera[nCnt].rotDest.y - g_camera[nCnt].rot.y>-0.01)
			{
				g_camera[nCnt].bWraparound = false;
			}
		}
		...
		// 向きの差を求める
		g_camera[nCnt].rotDiff.y = g_camera[nCnt].rotDest.y - g_camera[nCnt].rot.y;

		// 角度が一定値を超えた場合の数値制限
		if (g_camera[nCnt].rotDiff.y > D3DX_PI)
		{
			g_camera[nCnt].rotDiff.y -= D3DX_PI * 2;
		}
		...
		if (g_camera[nCnt].bWraparound == true)
		{
			g_camera[nCnt].rot.y += (g_camera[nCnt].rotDiff.y)*0.2f;
		}
		...
		g_camera[nCnt].posVDest.x = sinf(-g_camera[nCnt].rot.y) * g_camera[nCnt].fDistance;
		g_camera[nCnt].posVDest.y = 120;
		g_camera[nCnt].posVDest.z = -cosf(-g_camera[nCnt].rot.y) * g_camera[nCnt].fDistance;

		g_camera[nCnt].posR += (g_camera[nCnt].posRDest - g_camera[nCnt].posR)*0.1f;

		g_camera[nCnt].posV.x += (g_camera[nCnt].posVDest.x - g_camera[nCnt].posV.x)*0.1f;
		g_camera[nCnt].posV.z += (g_camera[nCnt].posVDest.z - g_camera[nCnt].posV.z)*0.1f;

		// 注視点の回転
		if (GetKeyboardPress(DIK_RIGHT) || nStickH > 0)
		{// 左旋回
			g_camera[nCnt].rotMove.y -= 0.005f;
		}
		...
		// 向きの更新
		g_camera[nCnt].rot += g_camera[nCnt].rotMove;

		// 慣性
		g_camera[nCnt].rotMove.y += (0 - g_camera[nCnt].rotMove.y)*0.125f;
		...
		{// 上
			g_camera[nCnt].posMove.y += CAMERA_MOVE;
		}
		...
		// 視点の場所の更新
		g_camera[nCnt].posV.y += g_camera[nCnt].posMove.y;

		if (g_camera[nCnt].posV.y <= 10)
		{
			g_camera[nCnt].posV.y = g_camera[nCnt].posVOld.y;
		}

		// 慣性
		g_camera[nCnt].posMove.y += (0 - g_camera[nCnt].posMove.y)*0.1f;
	}
}
